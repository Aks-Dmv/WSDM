...
TAU = 5*1e-1              # for soft update of target parameters
...
class Agent:
    ...
    def __init__(self,env, sess, state_size, action_size, random_seed):
        ...
        self.LRActor=LR_ACTOR
        self.LRCritic=LR_CRITIC
        self.incr=True
        ...
        self.actor_local.compile(loss='mse', optimizer=Adam(lr=self.LRActor))
        ...
        self.optimize = tf.train.AdamOptimizer(self.LRActor).apply_gradients(grads)
        ...
        self.critic_local.compile(loss='mse', optimizer=Adam(lr=self.LRCritic))
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        if (self.epsilon > FINAL_EPSILON):
            ...
            self.LRCritic -= (self.LRCritic-MIN_LR)/EPSILON_DECAY
            self.LRActor -= (self.LRActor-MIN_LR)/EPSILON_DECAY
        else:
            if(self.incr):
                print("incr done")
            self.incr=False
        # Slowly decay the learning rate
        # if ((self.epsilon < FINAL_EPSILON) and self.incr):
        #     self.epsilon -= (INITIAL_EPSILON-FINAL_EPSILON)/EPSILON_DECAY
        # else:
        #     if(self.incr):
        #         print("incr done")
        #     self.incr=False
        #
        # if ((self.epsilon > INITIAL_EPSILON) and not(self.incr)):
        #     self.epsilon += (INITIAL_EPSILON-FINAL_EPSILON)/EPSILON_DECAY
    ...
    def act(self, state,add_noise):
        ...
        if add_noise:
            ...
            The below is commented because we don't want noise on every step
            """
            ...
            #     action=noisyAction
            #     """
            #     Note: this part above, is only for perturbing the val
            #     """
            #     # action[:-1] = np.random.random_sample( self.action_size-1 )
            #     # action[:-1]=self.softmax(action[:-1])
