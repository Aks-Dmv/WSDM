...
class BFSWorldEnv:
    ...
    def step(self,action):
        ...
        if( softmaxOut==len(self._trueBoundaries) ):
            ...
            reward=10*self.penalty/(len(self.StateActionMemory)+1)
            # reward = self.penalty
        ...
        if(lessTh or greaterTh):
            ...
            if(lessTh):
                ...
                deltaBounds=10*(val-PrState[dim][0])+self.OutOfBounds
                #print("obo val, max/min dim", val,PrState[dim][0])

            if(greaterTh):
                deltaBounds=10*(PrState[dim][1]-val)+self.OutOfBounds
        ...
        if(math.isnan(val)):
            print("nan PrState, action",PrState, action)
        ...
        if(len(df.index)<=self.thresholdNumber):
            ...
            reward=(-1+action[-2])*10+self.penalty
